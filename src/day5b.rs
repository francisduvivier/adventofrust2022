...
pub fn solve(input_lines: Vec<String>) -> String {
    ...
    for mut i in 0..stacks.len() {
        if debug && !stacks[i].is_empty() { println!("stacks last before [{}]", stacks[i].last().unwrap()); }
        stacks[i].reverse();
        if debug && !stacks[i].is_empty() { println!("stacks first after [{}]", stacks[i].first().unwrap()); }
    }
    for mv in moves {
        if debug { println!("DO Move {} from {} to {}", mv.amount, mv.from, mv.to); }
        let origToSize = stacks[(mv.to - 1) as usize].len();
        for _ in 0..mv.amount {
            let popped = stacks[(mv.from - 1) as usize].pop().unwrap();
            if debug { println!("popped {} from {} to {}", popped, mv.from, mv.to) }
            stacks[(mv.to - 1) as usize].insert(origToSize, popped);
        }
    }
